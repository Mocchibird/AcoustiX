...
def reflection_coefficient(eta, cos_theta):
    ...
    """ reflection equation for sound

    Give reflection, calculate the eta (complex permittivity)

    abs(imag(eta)) = ln(2 / (1-reflection) - 1) / 2
    """
    
    imag_part_sqrt = tf.sqrt(tf.math.abs(tf.math.imag(eta)))

    r_te = -1.0 * tf.math.tanh(imag_part_sqrt)
    r_te = tf.cast(r_te, tf.complex64)
    r_tm = r_te
    return r_te, r_tm
