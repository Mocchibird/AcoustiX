...
class SolverPaths(SolverBase):
    ...
    * Fibonacci sampling, which find candidates by shooting and bouncing rays,
    ...
    def _spec_transition_matrices(self, relative_permittivity,
        ...
        # r_s = r_p, they are essentially the same with each other for acoustic reflection
        ...
        for depth in tf.range(0,max_depth):
            ...
            mat_cob = component_transform(last_e_r_s, last_e_r_p,
                                          e_i_s[depth], e_i_p[depth]) # compute the transform matrix from S and P polarization to Parallel and Perpendicular polarization
            mat_cob = tf.complex(mat_cob, tf.zeros_like(mat_cob))

            # No polarization transitions
            identity_matrix = tf.eye(2, batch_shape=[num_targets, num_sources, max_num_paths], dtype=self._dtype)
            # identity_matrix_complex = tf.complex(identity_matrix, tf.zeros_like(identity_matrix))

            # Only apply transform if valid reflection
            # [num_targets, num_sources, max_num_paths, 1, 1]
            valid__ = tf.expand_dims(valid_, axis=-1)
            # [num_targets, num_sources, max_num_paths, 2, 2]
            e = tf.where(valid__, tf.linalg.matmul(identity_matrix, mat_t), mat_t)
        ...
        if not scattering:
            ...
            # [num_targets, num_sources, max_num_paths, 2, 2]
            mat_t = tf.linalg.matmul(identity_matrix, mat_t)
            # print("Transfer matrices", mat_t.numpy()[0,0,:10,:,:])
    ...
    def _compute_paths_coefficients(self, rx_rot_mat, tx_rot_mat, paths,
        ...
        if n_scat > 0:
            ...
            e_in_pol = a_in_s*e_i_s + a_in_p*e_i_p  # scattering part was calculated here
